			...
			const struct i2c_device_id *id)
{
	...
	printk ( "SSD_gpios pointer = %d", SSD_gpios);
	...
	mdelay(5);
	gpio_set_value(UIB_TOUCH_RESET/*SSD_gpios[0]*/, 0);
	mdelay(5);
	gpio_set_value(UIB_TOUCH_RESET/*SSD_gpios[0]*/, 1);
	mdelay(25);
...
static const struct of_device_id ssd2543_ts_dt_ids[] = {
        { .compatible = "ssd,ssd2543", },
        { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, ssd2543_ts_dt_ids);
...
module_i2c_driver(ssd2543_driver);
